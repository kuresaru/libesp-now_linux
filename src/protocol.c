...
const static uint8_t radiotap_header_template[] = {0x00, 0x00, 0x12, 0x00, 0x2e, 0x48, 0x00, 0x00, 0x10, 0x02, 0x76,
                                                   0x09, 0xa0, 0x00, 0xd3, 0x00, 0x00, 0x00};
...
static uint8_t sendbuf[sizeof(radiotap_header_template) + sizeof(ieee80211_action_header_template) +
                       sizeof(ieee80211_wireless_management_header_template) + 250 + 4];
...
uint8_t esp_now_send(int sock_fd, const uint8_t *dest_mac, const uint8_t *data, uint8_t len) {
    ...
    // set length
    ((ieee80211_wireless_management_header_t *) (sendbuf + IEEE80211_WIRELESS_MANAGEMENT_HEADER_POSITION))->length =
            len + 5;
    // set data
            ...
            *from_mac = ((ieee80211_action_header_t *) (recvbuf + IEEE80211_ACTION_HEADER_POSITION))->src_mac;
            if (memcmp(
                ...
                *len = ((ieee80211_wireless_management_header_t *)
                        (recvbuf + IEEE80211_WIRELESS_MANAGEMENT_HEADER_POSITION))->length - 5;
